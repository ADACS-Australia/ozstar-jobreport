...
def main():
    ...
    for _ in range(args.num_tests):
        ...
        times = [
            future.result() for future in futures if isinstance(future.result(), float)
        ]
        ...
        print(
            f"{success_count}/{n} jobs succeeded in min/ave/max time: {min_time:.2f}/{avg_time:.2f}/{max_time:.2f} seconds"
        )
